...
def loss_function(recon_x, x, mu, logvar, kl_weight, free_bits=1.0):
    ...
    # Free Bits: 只有当 KL 大于阈值时才计算 Loss
    # 这能防止 KL 被压得太低 (Posterior Collapse)
...
def train_pipeline(args):
    ...
    for i in range(6):
        print(f"\n{'=' * 20} Training C-VAE Pipeline {i}/6 {'=' * 20}")
        ...
        # 【策略调整】
        target_kl_weight = 0.002  # 降低权重，给 z 呼吸空间
        target_free_bits = 1.0  # 允许 1.0 nat 的自由信息量
        initial_cond_drop = 0.5  # 初始强 Dropout
        ...
        for epoch in range(args.epochs):
            ...
            # Condition Dropout: 最终保留 0.1 的底噪，不降到 0
