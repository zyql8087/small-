    ...
    methods (Test)
        ...
        function testContinuousEvaluatorMatchesLegacyGrid(testCase)
            projected = project_method_constraints('M3', ...
                struct('c0', 0.04, 'c1', 0.10, 'c2', 0.16, 'w', 4), ...
                testCase.Config);
            legacy = build_graded_gyroid_field( ...
                projected, testCase.Config, 8);

            direct = evaluate_continuous_graded_gyroid(projected, ...
                testCase.Config, legacy.x_over_l, legacy.y_over_l, ...
                legacy.z_over_l);

            testCase.verifyEqual(direct.G, legacy.G);
            testCase.verifyEqual(direct.threshold, legacy.threshold);
            testCase.verifyEqual(direct.cell_size_over_l, ...
                legacy.cell_size_over_l);
        end
        ...
        function testContinuousEvaluatorClampsOnlyExteriorProfileInput(testCase)
            ...
            sample = evaluate_continuous_graded_gyroid(projected, ...
                testCase.Config, [-0.0625, 0.5, 1.0625], ...
                [-0.0625, 0.5, 1.0625], [-0.0625, 1, 2.0625]);
        ...
        function testContinuousEvaluatorRejectsNonIncreasingAxes(testCase)
            projected = project_method_constraints('M1', ...
                struct('c0', 0.10, 'c1', 0.10, 'c2', 0.10, 'w', 0), ...
                testCase.Config);

            TestFieldKernel.verifyFailure(testCase, ...
                @() evaluate_continuous_graded_gyroid(projected, ...
                testCase.Config, [0, 0.5, 0.25], [0, 1], [0, 2]), ...
                'MATLABGyroid:InvalidGeometry', ...
                'x_over_l must be strictly increasing');
        end
