...
def eval_inverse_diffusion(
    ...
    curve_np = prep.transform_y(y_raw)
    curve = torch.tensor(curve_np, dtype=torch.float32, device=DEVICE)
    timestep = torch.full((len(y_raw),), steps - 1, dtype=torch.long, device=DEVICE)
    best_x = None
    best_loss = None
    for _ in range(samples):
        x_t = torch.randn(len(y_raw), target_dim, dtype=torch.float32, device=DEVICE)
        model_output = model(x_t, timestep.float(), curve)
        if diffusion_config.get("prediction_type", config.get("diffusion_prediction_type", "epsilon")) == "x0":
            x0_pred = model_output
        else:
            x0_pred = predict_x0(x_t, timestep, model_output, schedule)
...
def eval_split_cont_diffusion(
    ...
    for _ in range(samples):
        x_t = torch.randn(len(y_raw), target_dim, dtype=torch.float32, device=DEVICE)
        model_output = model(x_t, timestep.float(), curve, cls_cond)
        if diffusion_config.get("prediction_type", config.get("diffusion_prediction_type", "epsilon")) == "x0":
            cont_pred = model_output
        else:
            cont_pred = predict_x0(x_t, timestep, model_output, schedule)
