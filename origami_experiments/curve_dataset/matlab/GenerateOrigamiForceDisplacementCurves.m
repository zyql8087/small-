...
addpath(genpath(fullfile(projectRoot, 'external', 'GenerateOrigamiDataSet')));
    ...
    try
        ...
        for conditionIndex = 1:height(conditions)
            ...
            curveTable = runLoadCase( ...
                ...
                row, finalLoad, finalLoadMode, targetLinearDisplacement, maxFinalLoad, plotOpen);
...
function curveTable = runLoadCase(ori, sampleId, curveId, loadCase, deployment, responseAxis, curvePoints, row, baseFinalLoad, finalLoadMode, targetLinearDisplacement, maxFinalLoad, plotOpen)
...
energyHistory = paddedHistoryMatrix(nr, 'strainEnergyHis', curvePoints, 4);
...
max_bar_stress = maxAbsHistory(nr, 'barSxHis', curvePoints);
max_bar_strain = maxAbsHistory(nr, 'barExHis', curvePoints);
max_crease_moment = maxAbsHistory(nr, 'sprMHis', curvePoints);
max_crease_rotation = maxAbsHistory(nr, 'sprRotHis', curvePoints);
...
function history = paddedHistoryMatrix(controller, propertyName, curvePoints, minColumns)
...
function values = maxAbsHistory(controller, propertyName, curvePoints)
