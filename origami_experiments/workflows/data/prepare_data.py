...
def main():
    ...
    expected_cols = len(INPUT_COLS) + len(OUTPUT_COLS)
    if data_all.shape[1] < expected_cols:
        raise ValueError(
            f"Expected at least {expected_cols} columns (8 input + 6 output), "
            f"but got {data_all.shape[1]}. Check input TXT file format."
        )
