...
from origami_experiments.workflows.analysis.audit_trained_models import MODEL_CANDIDATES
from origami_experiments.workflows.training.train_taskfit import TaskfitPreprocessor
...
def load_legacy_checkpoint(path: Path) -> dict:
    # Legacy checkpoints may pickle TaskfitPreprocessor objects whose class
    # reference is stored under a different module path.  Injecting the class
    # into globals() ensures torch.load can resolve it during unpickling.
